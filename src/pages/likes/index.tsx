...
type ColumnType = ColumnDef<LikeInterface, unknown>;

interface LikeListPageProps {
...
export function LikeListPage(props: LikeListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<LikeInterface>>(
    () => `/likes?params=${JSON.stringify(params)}`,
    fetcher,
  );
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/likes/view/${record.id}`} passHref legacyBehavior>
                <Button
