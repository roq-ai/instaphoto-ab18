...
function DashboardPage() {
  ...
  const likeStat = useSWR<PaginatedInterface<LikeInterface>>(
    () => `/likes`,
    () => getLikes({ limit: 1, offset: 0 }),
  );

  const commentStat = useSWR<PaginatedInterface<CommentInterface>>(
    () => `/comments`,
    () => getComments({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Likes',
      result: likeStat?.data?.totalCount,
      isLoading: likeStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Comments',
      result: commentStat?.data?.totalCount,
      isLoading: commentStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
