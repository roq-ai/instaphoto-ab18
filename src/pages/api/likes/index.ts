...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getLikesPublic() {
    ...
    const findOptions = convertQueryToPrismaUtil(query, 'like');
    const countOptions = omit(findOptions, 'include');
    const [totalCount, data] = await prisma.$transaction([
      prisma.like.count(countOptions as unknown),
  ...
  async function getLikes() {
    ...
    const limit = parseInt(_limit as string, 10) || 20;
    const offset = parseInt(_offset as string, 10) || 0;
    const response = await prisma.like
      .withAuthorization({
        roqUserId,
      ...
      .findManyPaginated({
        ...convertQueryToPrismaUtil(query, 'like'),
        take: limit,
        skip: offset,
        ...(order?.length && {
          orderBy: getOrderByOptions(order),
