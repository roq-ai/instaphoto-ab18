...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getCommentsPublic() {
    const {
      limit: _limit,
      offset: _offset,
      order,
    ...
    const findOptions = convertQueryToPrismaUtil(query, 'comment');
    const countOptions = omit(findOptions, 'include');
    const [totalCount, data] = await prisma.$transaction([
      prisma.comment.count(countOptions as unknown),
  ...
  async function getComments() {
    ...
    const response = await prisma.comment
      ...
      .findManyPaginated({
        ...convertQueryToPrismaUtil(query, 'comment'),
        take: limit,
        skip: offset,
        ...(order?.length && {
          orderBy: getOrderByOptions(order),
